...
```{r}
july_ggplot = subset(ny_noaa_plot, month == "July") %>% 
  ggplot(aes(x = year, y = mean_tmax, group = station, color = station)) +
  geom_line() +
  theme(legend.position = "none") +
  labs(
    title = "Average Maximum Temperature in July by Year and Weather Station in NY",
    x = "Year",
    y = "Average Maximum Temperature in Celsius"
  ) +
  scale_y_continuous(
    breaks = c(10, 15, 20, 25, 30, 35, 40), 
    labels = c("10 C", "15 C", "20 C", "25 C", "30 C", "35 C", "40 C")) +
  viridis::scale_color_viridis(
    name = "station", 
    discrete = TRUE,
    option = "B"
